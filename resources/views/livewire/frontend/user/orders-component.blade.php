            ...
            <div x-show="showOrder" x-on:click.away="showOrder = false" class="border rounded shadow p-4">
                <div class="table-responsive mb-4">
                    <table class="table">
                        ...
                        <tr>
                            <th class="border-0" scope="col"><strong class="text-small text-uppercase">Produto</strong></th>
                            <th class="border-0" scope="col"><strong class="text-small text-uppercase">Valor</strong></th>
                            <th class="border-0" scope="col"><strong class="text-small text-uppercase">Quantidade</strong></th>
                        ...
                        @if(!is_null($order->discount_code))
                            <tr>
                                <td colspan="3" class="text-right"><strong>Desconto (<small>{{ $order->discount_code }}</small>)</strong></td>
                                <td>{{ $order->currency() . ' ' . number_format($order->discount, 2) }}</td>
                            </tr>
                        @endif
                        <tr>
                            <td colspan="3" class="text-right"><strong>Taxa</strong></td>
                            <td>{{ $order->currency() . ' ' . number_format($order->tax, 2) }}</td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-right"><strong>Entrega</strong></td>
                ...
                <h2 class="h5 text-uppercase">Taansação</h2>
                <div class="table-responsive mb-4">
                    <table class="table">
                        <thead class="bg-light">
                        <tr>
                            <th class="border-0" scope="col"><strong class="text-small text-uppercase">Taansação</strong></th>
                            <th class="border-0" scope="col"><strong class="text-small text-uppercase">Datq</strong></th>
                       {{-- <th class="border-0" scope="col"><strong class="text-small text-uppercase">Days</strong></th> --}}
                        ...
                        @forelse($order->transactions as $transaction)
                           ...
                           {{-- <td>{{ \Carbon\Carbon::now()->addDays(5)->diffInDays($transaction->created_at) }}</td>--}}
                                <td>
                                    @if ($loop->last && $transaction->transaction_status == \App\Models\OrderTransaction::FINISHED &&
                                        \Carbon\Carbon::now()->addDays(5)->diffInDays($transaction->created_at) != 0)
