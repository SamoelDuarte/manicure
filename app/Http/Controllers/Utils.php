...
class Utils extends Controller
{
    ...
    static function removeTags($html) {
        // Remove as tags HTML e substitui o fechamento da tag por \n
        $text = preg_replace('/<\/[^>]*>/i', "\n",$html);
    ...
    public static function  getAddresComplet($id)
    {
        $cep =  UserAddress::find($id);

        $viaCepUrl = "https://viacep.com.br/ws/{$cep->zipcode}/json/";
    ...
    public static function sanitizeItAll($str)
    {
    ...
    public static function getDDD($phoneNumber)
    {
        $areaCode = substr($phoneNumber, 0, 2); // Extrai os dois dígitos após o DDI (55)
        return $areaCode;
    }

    public static function PhoneWithoutDDD($fullPhoneNumber)
    {
        $phoneNumber = substr($fullPhoneNumber, 2); // Remove o DDI (55) e o DDD (11)
        return $phoneNumber;
    }
    public static function createRecruitmentCode()
    {
        return Str::uuid();
    }
    ...
    public static function formatPrice($price)
    {
        ...
        $originalValue = str_replace(",", ".", str_replace(".", "", $price));
        $convertedValue = intval($originalValue * 100);

        return  $convertedValue; // Isso imprimirá 9700
    }
