<?php

namespace App\Http\Controllers\Backend;

use App\Http\Controllers\Controller;
use App\Http\Requests\Backend\CategoryRequest;
use App\Models\Category;
use App\Models\MercadoLivreData;
use App\Traits\ImageUploadTrait;
use GuzzleHttp\Client;
use Illuminate\Contracts\View\View;
use Illuminate\Http\RedirectResponse;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\File;

class CategoryController extends Controller
{
    use ImageUploadTrait;

    public function index(): View
    {
        $this->authorize('access_category');

        $categories = Category::with('parent')
            ->withCount('products')
            ->when(\request()->keyword != null, function ($query) {
                $query->search(\request()->keyword);
            })
            ->when(\request()->status != null, function ($query) {
                $query->whereStatus(\request()->status);
            })
            ->orderBy(\request()->sortBy ?? 'id', \request()->orderBy ?? 'asc')
            ->paginate(\request()->limitBy ?? 10);

        return view('backend.categories.index', compact('categories'));
    }

    public function create(): View
    {
        $this->authorize('create_category');
        $mainCategories = Category::whereNull('parent_id')->get(['id', 'name']);
        $token = MercadoLivreData::where('user_id', Auth::user()->id)->first();


        // Obtenha as categorias da API do Mercado Livre
        $client = new Client();
        $response = $client->get('https://api.mercadolibre.com/sites/MLB/categories');
        $categorias = json_decode($response->getBody(), true);

        // Envie as categorias para a view
        return view('backend.categories.create', compact('mainCategories', 'categorias'));
    }

    public function store(CategoryRequest $request): RedirectResponse
    {
        $this->authorize('create_category');
        $parentCategory = Category::where('name', $request->parent_id)->first();
        $image = NULL;

      
        // dd($request->all());
        if ($parentCategory == null) {
            if ($request->hasFile('cover')) {
                $image = $this->uploadImage($request->parent_id, $request->cover, 'categories', 500, NULL);
            }
           $parentCategory =  Category::create([
                'name' => $request->parent_id,
                'status' => $request->status,
                'cover' => $image
            ]);

        } 
        
        if ($request->hasFile('cover')) {
            $image = $this->uploadImage($request->name, $request->cover, 'categories', 500, NULL);
        }

        Category::create([
            'name' => $request->name,
            'category_ml' => $request->subcategory_id,
            'parent_id' => $parentCategory->id,
            'status' => $request->status,
            'cover' => $image
        ]);


        clear_cache();

        return redirect()->route('admin.categories.index')->with([
            'message' => 'Created successfully',
            'alert-type' => 'success'
        ]);
    }

    public function show(Category $category): View
    {
        $this->authorize('show_category');

        return view('backend.categories.show', compact('category'));
    }

    public function edit(Category $category): View
    {
        $this->authorize('edit_category');

        $mainCategories = Category::whereNull('parent_id')->get(['id', 'name']);

        return view('backend.categories.edit', compact('category', 'mainCategories'));
    }

    public function update(CategoryRequest $request, Category $category): RedirectResponse
    {
        $this->authorize('edit_category');

        $image = $category->cover;
        if ($request->has('cover')) {
            if ($category->cover != null && File::exists('storage/assets/images/categories/' . $category->cover)) {
                unlink('storage/assets/images/categories/' . $category->cover);
            }
            $image = $this->uploadImage($request->name, $request->cover, 'categories', 500, NULL);
        }

        $category->update([
            'name' => $request->name,
            'slug' => null,
            'parent_id' => $request->parent_id,
            'status' => $request->status,
            'cover' => $image
        ]);

        clear_cache();
        return redirect()->route('admin.categories.index')->with([
            'message' => 'Updated successfully',
            'alert-type' => 'success'
        ]);
    }

    public function destroy(Category $category): RedirectResponse
    {
        $this->authorize('delete_category');

        if ($category->cover) {
            if (File::exists('storage/assets/images/categories/' . $category->cover)) {
                unlink('storage/assets/images/categories/' . $category->cover);
            }
        }

        $category->delete();

        clear_cache();
        return redirect()->route('admin.categories.index')->with([
            'message' => 'Deleted successfully',
            'alert-type' => 'success'
        ]);
    }

    public function removeImage(Category $category): RedirectResponse
    {
        $this->authorize('delete_category');

        if (File::exists('storage/images/categories/' . $category->cover)) {
            unlink('storage/images/categories/' . $category->cover);
            $category->cover = null;
            $category->save();
        }

        clear_cache();
        return back()->with([
            'message' => 'Image deleted successfully',
            'alert-type' => 'success'
        ]);
    }
}
