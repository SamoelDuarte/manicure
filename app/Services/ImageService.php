...
class ImageService
{
    ...
    public function storeProductImages($images, $product, $i = 1)
    {
        foreach ($images as $image) {
            $product->media()->create([
                'file_name' => $this->uploadImages($product->name, $image, $i, 'products', 500, 500),
    ...
    public function storeBannerImages($fileName, $image): string
    {
        return $this->uploadImage(
            $fileName,
            $image,
    ...
    {  
        if (File::exists('storage/images/'. $folderName .'/' . $image)) {
            ...
            File::delete('storage/images/'. $folderName .'/' . $image);
        }
    }
}
